...
class PlaylistViewGetTests(APITestCase):
    ...
    def setUpTestData(cls):
        cls.url = reverse('api-playlists')
        cls.user = create_test_user()
        cls.playlist = Playlist.objects.create(name='test_playlist',
                                               added_by=cls.user)
        cls.media_link_1 = MediaLink.objects.create(url='test.url',
                                                    added_by=cls.user)
        cls.media_link_2 = MediaLink.objects.create(url='test.url2',
                                                    added_by=cls.user)
...
class PlaylistDetailViewTests(APITestCase):
    ...
    def test_get_playlist_detail_element_fields(self):
        ...
        self.assertIn('url', response.data['elements'][0]['media_link'])
...
class MediaLinkViewGetTests(APITestCase):
    ...
    @classmethod
    def setUpTestData(cls):
        cls.url = reverse('api-media_links')
        cls.user = create_test_user()
        cls.media_link_1 = MediaLink.objects.create(url='test.url',
                                                    added_by=cls.user)
        cls.media_link_2 = MediaLink.objects.create(url='test.url2',
...
class MediaLinkPostTests(APITestCase):
    ...
    def test_post_authenticated_response_code(self):
        self.client.login(username=self.username, password=self.password)
        response = self.make_request(dict(url='test.url'))
        
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    
    def test_post_creates_media_link(self):
        self.client.login(username=self.username, password=self.password)
        self.make_request(dict(url='test.url'))
        
        self.assertTrue(
            MediaLink.objects.filter(url='test.url').exists()
        )
...
class MediaLinkDetailViewTests(APITestCase):
    ...
    @classmethod
    def setUpTestData(cls):
        cls.view_name = 'api-media_links-detail'
        cls.user = create_test_user()
        cls.media_link_1 = MediaLink.objects.create(url='test.url',
                                                    added_by=cls.user)
    ...
    def test_put_media_link_detail(self):
        ...
        self.assertEqual(self.media_link_1.url, 'new_url')
